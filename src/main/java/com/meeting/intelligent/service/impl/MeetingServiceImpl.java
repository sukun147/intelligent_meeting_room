...
public class MeetingServiceImpl extends ServiceImpl<MeetingDao, MeetingEntity> implements MeetingService {
    ...
    @PostConstruct
    public void init() {
        QuickConfig qc = QuickConfig.newBuilder("meeting_cache_").expire(Duration.ofHours(1)).cacheType(CacheType.REMOTE).build();
        meetingCache = cacheManager.getOrCreateCache(qc);
    }

    @Override
    public PageUtils queryPage(Map<String, Object> params) {
        IPage<MeetingRespVo> page = this.page(new Query<MeetingEntity>().getPage(params), new QueryWrapper<>()).convert(meetingEntity -> {
        ...
        Map<Long, String> roomMap = meetingRoomService.listByIds(roomIds).stream().collect(HashMap::new, (m, v) -> m.put(v.getRoomId(), v.getPosition()), HashMap::putAll);
    ...
    public void add(MeetingVo meetingVo) {
        ...
        try {
            ...
            throw new GlobalException(SCHEDULER_EXCEPTION.getMsg(), SCHEDULER_EXCEPTION.getCode());
        }
    }
    ...
    public MeetingRespVo get(Long meetingId) {
        ...
        meetingCache.put(meetingId.toString(), meetingRespVo, 55 + new Random().nextInt(10), TimeUnit.MINUTES);
        return meetingRespVo;
    }
    ...
    public List<MeetingRespVo> getByRoomId(Long roomId) {
        QueryWrapper<MeetingEntity> wrapper = new QueryWrapper<MeetingEntity>().eq("room_id", roomId).eq("meeting_status", 1);
    ...
    public void updateForce(MeetingVo meetingVo) {
        ...
        this.updateById(meetingEntity);
        remind(meetingEntity, meetingRoomService.getById(meetingEntity.getRoomId()).getPosition(), meetingEntity.getStartTime());
    ...
    public void reserveMeeting(MeetingVo meetingVo, long createTime) {
        Long roomId = meetingVo.getRoomId();
        Date startTime = meetingVo.getStartTime();
        Date endTime = meetingVo.getEndTime();
        MeetingEntity meetingEntity = new MeetingEntity();
        BeanUtils.copyProperties(meetingVo, meetingEntity);
        QueryWrapper<MeetingEntity> nowRoomQuery = new QueryWrapper<MeetingEntity>().eq("room_id", roomId).eq("meeting_status", 1).and(wrapper -> wrapper.between("start_time", startTime, endTime).or().between("end_time", startTime, endTime));
        ...
        if (conflictMeetings.size() > 0) {
            if (conflictMeetings.stream().anyMatch(meeting -> meeting.getScheduledPeriod() != null)) {
                throw new GlobalException(CONFLICT_WITH_PERIODIC_MEETING_EXCEPTION.getMsg(), CONFLICT_WITH_PERIODIC_MEETING_EXCEPTION.getCode());
            }
            if (startTime.getTime() - createTime < TWO_DAY) {
            ...
            saveMeeting(meetingEntity, roomId, startTime);
        }
    }

    private void saveMeeting(MeetingEntity meetingEntity, Long roomId, Date startTime) {
        ...
        remind(meetingEntity, meetingRoomService.getById(roomId).getPosition(), startTime);
    }

    private void remind(MeetingEntity meetingEntity, String position, Date startTime) {
        ...
        try {
            ...
            Trigger trigger = TriggerBuilder.newTrigger().startAt(new Date(startTime.getTime() - THIRTY_MINUTES)).build();
            scheduler.scheduleJob(jobDetail, trigger);
        } catch (SchedulerException e) {
    ...
    private void adjustMeeting(List<MeetingEntity> conflictMeetings, MeetingEntity currentMeeting) {
        ...
        PriorityQueue<MeetingEntity> priorityQueue = new PriorityQueue<>(Comparator.comparingInt(o -> (int) ((o.getEndTime().getTime() - o.getStartTime().getTime()) / 1000 / 60 / 60 * o.getParticipants().size() + (o.getScheduledPeriod() != null ? 100 : 0))));
        ...
        saveMeeting(priorityMeeting, roomId, startTime);
        ...
        QueryWrapper<MeetingEntity> targetDayMeetings = new QueryWrapper<MeetingEntity>().eq("room_id", roomId).eq("meeting_status", 1).between("start_time", idleStartTime, idleEndTime).between("end_time", idleStartTime, idleEndTime);
        ...
        while (!priorityQueue.isEmpty()) {
            ...
            for (Times times : subList) {
                if (meeting.getEndTime().getTime() - meeting.getStartTime().getTime() <= times.getEndTime().getTime() - times.getStartTime().getTime()) {
                    ...
                    saveMeeting(meeting, roomId, times.getStartTime());
                    break;
                }
            }
